...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.av.com/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.av.com/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.av.com/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.av.com/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.av.com/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.av.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.av.com-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.av.com-cert.pem"
...
function orderer_cert() {
    ...
    mkdir -p organizations/ordererOrganizations/av.com/ca
    mkdir -p organizations/ordererOrganizations/av.com/msp/cacerts
    mkdir -p organizations/ordererOrganizations/av.com/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/av.com/orderers
    mkdir -p organizations/ordererOrganizations/av.com/tlsca

    CERT_DIR=organizations/ordererOrganizations/av.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.av.com-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.av.com-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/av.com/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/av.com/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.av.com-cert.pem"
